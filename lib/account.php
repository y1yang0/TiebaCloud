...
if (isset($_POST['reg'])) {
	if (strlen($_POST['username']) < 6 || strlen($_POST['password']) < 6 || preg_match("/[\x7f-\xff]/", $_POST['username'])) {
		...
		if (!$con) {
			...
			if (mysqli_select_db($con, DB_NAME)) {
				if ($res = mysqli_fetch_array(mysqli_query($con, 'SELECT * FROM tc_user WHERE username="' . $_POST['username'] . '"'))) {
					error_tpl('贴吧云注册错误', '你输入的用户名已经存在 :(', '../register.php');
				} else {
					mysqli_query($con, 'set names utf8');
					mysqli_query($con, 'INSERT INTO tc_user(username,password) VALUES("' . $_POST['username'] . '","' . md5($_POST['password']) . '")');
					mysqli_query($con, 'INSERT INTO tc_baiduinfo(tc_id) VALUES("' . $_POST['username'] . '")');
	...
	if (!$con) {
		...
		if (mysqli_select_db($con, DB_NAME)) {
			$ret = mysqli_query($con, 'SELECT * FROM tc_user WHERE username ="' . $_POST['log_username'] . '"AND password ="' . md5($_POST['log_password']) . '"');
			if ($res = mysqli_fetch_array($ret)) {
				session_start();
