...
class BaiduUtil {
	...
	protected function fetch($url, $mobile = TRUE, $usecookie = TRUE) {
		...
		if ($mobile === TRUE) {
			if (!array_key_exists('error_code', $result)) {
				throw new Exception('未收到正确数据', -11);
			}
	...
	protected function commonReturn($data) {
		$result = array();
		if (isset($data['no']) && !isset($data['errorcode'])) {
			$data['error_code'] = $data['no'];
		}

		if (isset($data['error']) && !isset($data['error_msg'])) {
	...
	public function fetchWebTbs() {
		...
		if (array_key_exists('is_login', $result) === TRUE && $result['is_login'] === 0) {
			throw new Exception('获取webtbs失败', -14);
		}
...
EOF;
	...
	public function getForumInfo($kw, $type = 'forum') {
		if (!array_key_exists($kw, $this->forumPages)) {
			$this->fetchForumPage($kw);
		}
		...
		case 'post':
			...
			foreach ($forum['tlist'] as $thread) {
				if ($thread['is_top'] == 0 && $thread['is_posted'] == 0) {
					$post_threads[] = $thread;
				}
		...
		case 'zan':
			$zan_threads = array();
			foreach ($forum['tlist'] as $thread) {
				if (!isset($thread['is_zaned'])) {
					throw new Exception("没有点赞信息", -18);
				}

				if ($thread['is_top'] == 0 && $thread['is_zaned'] == 0) {
					$zan_threads[] = $thread;
				}

			}
			if (!count($zan_threads)) {
				throw new Exception('无可赞的帖子', -12);
			}
	...
	public function multiSign() {
		try {
			...
			$forum_ids = '';
			if (!@count($forums)) {
				throw new Exception("没有可以一键签到的贴吧", -17);
			}
	...
	public function post($kw, $fid = NULL, $tid = NULL, $content = NULL) {
		try {
			...
			if (is_null($tid)) {
				$tid = $this->getForumInfo($kw, 'post');
			}

			if (is_null($content)) {
				$content = self::getRandomContent();
			}
		...
		try {
			if ($free === FALSE) {
				...
				if ($result['data']['new_price'] != 0) {
					throw new Exception('免费抽奖机会已经用完', -16);
				}
